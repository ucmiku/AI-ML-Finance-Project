...
RETURN_DIMS = [
    ...
    ('Return_Avg_Trade_Pnl',      'higher', 0.15, '笔均盈亏 (Avg Trade PnL)'),
]
...
def compute_scores(strategies_results):
    ...
    for name, res in strategies_results.items():
        ...
        raw[name] = {
            ...
            'Return_Avg_Trade_Pnl':      float(m['Avg_Trade_Pnl']),  # dup for category isolation
...
def main():
    ...
    final_scores_2025, ranking_2025, raw_2025 = compute_scores(results)

    # ==========================================
    # 2026 H1 — Frozen-rule scoring (separate, no re-optimisation)
    # ==========================================
    market_26, pred_26 = load_c1_unified_2026()
    engine_26 = ERCOTBacktestEngine(
        market_26, initial_capital=100000,
        fee_per_mwh=2.0, slippage_bps=50.0, capture_rate=0.65,
    )

    results_2026 = {}
    results_2026['B2B_Baseline_060']    = engine_26.execute_b2b_baseline(pred_26, threshold=0.60)
    results_2026['B2B_Optimal_070']     = engine_26.execute_b2b_baseline(pred_26, threshold=0.70)
    results_2026['ExtremeWeather_Only'] = engine_26.execute_b2b_b2a_combined(pred_26, threshold=0.60, use_b2a_direction=True, extreme_weather_filter=True)

    final_scores_2026, ranking_2026, raw_2026 = compute_scores(results_2026)

    # --- Print 2025 report ---
    def print_report(final_scores, ranking, period_label):
        def bar(val, width=20):
            n = int(round(val / 100 * width))
            return '#' * n + '-' * (width - n)

        print(f'\n\n{"=" * 110}')
        print(f'  {period_label} — Multi-Dimensional Strategy Scoring Report')
        print(f'{"=" * 110}')
        print()
        ...
        for rank, name in enumerate(ranking, 1):
            ...
            cats = fs['categories']
            print(f'  #{rank:<3d} {name:<28s} {fs["composite"]:>5.1f}  '
                  f'{cats["收益能力 (Return)"]   ["score_0_100"]:>9.1f}  '
                  f'{cats["风险控制 (Risk)"]     ["score_0_100"]:>9.1f}  '
                  f'{cats["稳健性 (Robustness)"] ["score_0_100"]:>9.1f}  '
                  f'{cats["交易效率 (Efficiency)"]["score_0_100"]:>9.1f}')

        # Detailed breakdown
        print(f'\n{"=" * 110}')
        print(f'  {period_label} — Per-Strategy Dimension Details')
        print(f'{"=" * 110}')
        ...
        for rank, name in enumerate(ranking, 1):
            fs = final_scores[name]
            print(f'\n  #{rank} {name}  --  Composite: {fs["composite"]:.1f}/100  {bar(fs["composite"])}')
            print(f'  {"Dimension":<32s} {"Raw":>12s}  {"Score":>6s}  {"Distribution"}')
    ...
    print_report(final_scores_2025, ranking_2025, '2025 OOF (Development)')
    print_report(final_scores_2026, ranking_2026, '2026 H1 (Frozen-Rule Test)')

    # Export JSON — separate 2025 and 2026
    def build_export(final_scores, ranking, raw, results_dict):
        export = {
            'scoring_methodology': {
    ...
    export_2025 = build_export(final_scores_2025, ranking_2025, raw_2025, results)
    export_2026 = build_export(final_scores_2026, ranking_2026, raw_2026, results_2026)

    full_export = {
        'scoring_methodology': export_2025['scoring_methodology'],
        '2025': export_2025,
        '2026': export_2026,
    }
