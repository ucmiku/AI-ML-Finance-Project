...
> **交付对象**：成员 D（全栈/前端开发）
> **数据文件**：`bk_testing/backtest_result_c1.json`（1,330 KB）
> **生成脚本**：`bk_testing/generate_frontend_data.py`
> **更新频率**：策略参数变更后重新运行 `generate_frontend_data.py` 即可
> **最新更新**：已包含 2025 OOF + 2026 H1 Walk-Forward 双时段数据
...
{
  "meta":                      { ... },  // 项目元信息 + 双时段描述
  "2025": {                    // === 2025 OOF（开发与参数优化） ===
    "strategy_comparison":     [ ... ],  // 7 策略核心指标对比
    "equity_curves":           { ... },  // 7 策略每日净值曲线
    "trades":                  { ... },  // 各策略交易明细
    "monthly_pnl":             { ... },  // 各策略月度 PnL
    "risk_reports":            { ... },  // 各策略风险集中度
    "market_data":             { ... },  // 市场背景（日度均价差）
    "threshold_sensitivity":   [ ... ]   // B2B 阈值敏感性曲线
  },
  "2026": {                    // === 2026 H1（冻结规则时序检验） ===
    "strategy_comparison":     [ ... ],  // 3 策略（仅冻结的 top 3）
    "equity_curves":           { ... },
    "trades":                  { ... },
    "monthly_pnl":             { ... },
    "risk_reports":            { ... },
  ...
  "cross_period_comparison":   [ ... ],  // 2025 vs 2026 同策略跨期对比
  "strategy_scoring":          { ... }   // 多维度评分（含 2025 + 2026 分别排名）
}
```

> **⚠️ 结构变更**：原顶层 `strategy_comparison`/`equity_curves` 等字段已迁移到 `2025` 和 `2026` 子对象中。请更新前端数据读取路径：
> ```js
> // 旧: data.strategy_comparison
> // 新: data["2025"].strategy_comparison 或 data["2026"].strategy_comparison
> // 跨期对比: data.cross_period_comparison
> ```
...
| 新增字段 | 位置 | 用途 |
|---|---|---|
| `meta.periods` | `meta` 内 | 双时段描述（类型/来源/小时数/用途） |
| `cross_period_comparison` | 顶层 | 3 策略 × 2 时段的指标对比 + `change` 方向标识 |
| `strategy_scoring.2025_ranking` / `2026_ranking` | `strategy_scoring` 内 | 两个时段分别的评分排名 |
| `2026.market_data.total_weeks` | `2026.market_data` 内 | Walk-Forward 周数（27） |
